...
module BattleCommand_Custom
  ...
  ACTOR_COMMAND_LIST = [:attack, :guard, :skill, :item, :state, :equip_change, :escape,:M5_party]
...
class Window_ActorCommand < Window_Command
  ...
  def make_command_list_by_symbol(symbol)
    ...
    when :state     ; add_state_command
    when :equip_change ; add_equip_change_command  
    end
  end
