...
module EasyCompose
  ...
  Text = "Используйте кнопку X для переключения"
...
#~   end
  ...
  def make_command_list
    make_command_list_easy_compose
    return unless $game_switches[EasyCompose::SID] || $game_switches[EasyCompose::SID_1] || $game_switches[EasyCompose::SID_2]
    a = []
    @list.each_with_index {|c,i| a.push(i) if c[:symbol] == :buy}
    if $game_switches[EasyCompose::SID]
      a.each {|i| @list[i][:name] = EasyCompose::Name}
    elsif $game_switches[EasyCompose::SID_1]
      a.each {|i| @list[i][:name] = EasyCompose::Name_1}
    elsif $game_switches[EasyCompose::SID_2]
      a.each {|i| @list[i][:name] = EasyCompose::Name_2}
    end
  end
end
...
#~     f = $game_switches[EasyCompose::SID] ? true : false
    f = $game_switches[EasyCompose::SID] || $game_switches[EasyCompose::SID_1] || $game_switches[EasyCompose::SID_2] ? true : false
    @compose_mode_main = f
    @compose_mode = f
...
#      if $game_switches[EasyCompose::SID]
      if $game_switches[EasyCompose::SID] || $game_switches[EasyCompose::SID_1] || $game_switches[EasyCompose::SID_2]
        Sound.play_cursor
        @compose_mode ^= true
        @page_index = 0
        refresh
...
#~     return true unless $game_switches[EasyCompose::SID]
    return true unless $game_switches[EasyCompose::SID] || $game_switches[EasyCompose::SID_1] || $game_switches[EasyCompose::SID_2]
    item.easy_compose_item_list.each {|k1,v1|
    v1.each {|k2,v2|
    case k1
    when 0;it = $data_items[k2]
  ...
  def draw_item(index)
    ...
    if !($game_switches[EasyCompose::SID] || $game_switches[EasyCompose::SID_1] || $game_switches[EasyCompose::SID_2]) or price(item) > 0
...
#~     return r1 unless $game_switches[EasyCompose::SID]
    return r1 unless $game_switches[EasyCompose::SID] || $game_switches[EasyCompose::SID_1] || $game_switches[EasyCompose::SID_2]
    ra = []
    ra.push(r1)
    @item.easy_compose_item_list.each {|k1,v1|
    v1.each {|k2,v2|
...
class RPG::BaseItem
  ...
  def easy_compose_item_list
    ...
    if $game_switches[EasyCompose::SID] 
      ...
      @easy_compose_item_list = create_easy_compose_item_list_1
    elsif $game_switches[EasyCompose::SID_2]
      @easy_compose_item_list = create_easy_compose_item_list_2
    end
  end
  #--------------------------------------------------------------------------
  # 合成素材リストの作成
  ...
  def create_easy_compose_item_list_1
    list = {}
    list[0] = {}
    list[1] = {}
    list[2] = {}
    w = EasyCompose::Word_1
    note.each_line {|l|
    if /#{w}[:：](\d+),(\S+)/ =~ l
      a = [$1.to_i] + $2.to_s.split(/\s*,\s*/).inject([]) {|r,i| r.push(i.to_i)}
      if a[0] < 0 or a[0] > 2
  ...
  # 合成素材リストの作成
  #--------------------------------------------------------------------------
  def create_easy_compose_item_list_2
    list = {}
    list[0] = {}
    list[1] = {}
    list[2] = {}
    w = EasyCompose::Word_2
    note.each_line {|l|
    if /#{w}[:：](\d+),(\S+)/ =~ l
      a = [$1.to_i] + $2.to_s.split(/\s*,\s*/).inject([]) {|r,i| r.push(i.to_i)}
      if a[0] < 0 or a[0] > 2
